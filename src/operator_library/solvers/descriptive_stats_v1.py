"""DEPRECATED — kept for V8 Phase-2 rollback only.  Do not import.

This file is the pre-§2.3 snapshot of ``descriptive_stats.py``.  The
live solver registered as ``describe_full`` / ``distribution_histogram``
is in ``descriptive_stats.py``; this copy exists so we can diff /
revert if the new sniff + fail-fast logic breaks anything in
production.  Safe to delete after two stable benchmark cycles.
