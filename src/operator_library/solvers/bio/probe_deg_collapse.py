...
CONTRACT = SolverContract(
    ...
    output_kind={"gene_deg_table_csv": "s"},  # rows = genes with DE stats
)
...
class ProbeDegCollapseToGeneSolver:
    ...
    def run(self, df: pd.DataFrame, mapping: ColumnMapping,
        ...
        #
        # Edge case: caller's DEG table has a single id column literally
        # named ``gene_symbol`` and no separate ``probe_id``.  If an
        # annotation table is supplied, prefer that external probe→gene
        # mapping: GEO/GEO2R-style exports often label the tested probe id
        # column as "gene_symbol" even when values are Agilent probe IDs
        # (A_51_..., A_52_...).  Without annotation, preserve the previous
        # permissive behaviour and trust the caller's column label.
        only_id_col_named_gene_symbol = (
            probe_col == "gene_symbol"
            and "gene_symbol" not in [c for c in deg.columns if c != probe_col]
        )
        deg = deg.rename(columns={probe_col: "probe_id"}).copy()
        deg["probe_id"] = deg["probe_id"].astype(str)
        restore_gene_symbol_from_label = (
            only_id_col_named_gene_symbol and not ann_path
        )
        if restore_gene_symbol_from_label and "gene_symbol" not in deg.columns:
            deg["gene_symbol"] = deg["probe_id"]
        ...
        elif restore_gene_symbol_from_label:
            # Restored above from the caller's labeled column.
            symbol_series = deg.set_index("probe_id")["gene_symbol"]
            gene_source = "deg_inline_restored_from_probe_col_label"
        else:
            if not ann_path:
                raise ValueError(
