...
Live module is ``g_formula_tmle.py``.  Pre-§2.3 implementation kept
in case the new outcome/shape sniff breaks legitimate use.  Safe to
delete after two stable benchmark cycles.
...
CONTRACT = SolverContract(
    ...
    description=(
        "Causal Average Treatment Effect (ATE) of a binary treatment on a "
        "binary outcome via three estimators: parametric G-formula (Robins "
        "1986 standardization), Inverse-Probability-of-Treatment Weighting "
        "(IPTW), and Targeted Maximum Likelihood Estimation (TMLE, van der "
        "Laan 2006 — double-robust).  Useful for non-randomised / real-world "
        "evidence comparisons.  Inputs: binary treatment column, binary "
        "outcome column, list of covariate columns (confounders to adjust)."
    ),
    roles={
...
class GFormulaTmleSolver:
    ...
    def run(self, df: pd.DataFrame, mapping: ColumnMapping,
        ...
        if not A_col or not Y_col or not L_cols:
            raise ValueError("treatment_col, outcome_col, confounders required")
