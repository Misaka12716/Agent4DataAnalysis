...
class PCADecomposeSolver:
    ...
    def run(self, df: pd.DataFrame, mapping: ColumnMapping,
        ...
        # SVD-based PCA (mean already removed if standardised; remove
        # column means again to be safe)
        Xc = X - X.mean(axis=0, keepdims=True)
        U, S, Vt = np.linalg.svd(Xc, full_matrices=False)
        # principal components: columns of V; scores = U * S
        scores = (U[:, :k] * S[:k])         # (n_samples, k)
        loadings = Vt[:k, :]                # (k, n_genes)
        # variance explained
