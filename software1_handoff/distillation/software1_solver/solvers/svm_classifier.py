...
CONTRACT = SolverContract(
    ...
    description=(
        "RBF-kernel SVM with GridSearchCV over C × gamma and 5-fold "
        "stratified CV (seed=42).  Output predictions.csv and "
        "metrics.json (auroc, accuracy, best_params)."),
    roles={
        "id_col":         RoleSpec(Role.ID, "patient identifier"),
        "feature_columns": RoleSpec(
            Role.NUMERIC_LIST,
            "the 8 numeric feature columns",
        ),
        "target_col": RoleSpec(
            Role.BINARY_TARGET, "0/1 target if present in input",
            optional=True),
    },
    static_params={
        "random_state": 42,
