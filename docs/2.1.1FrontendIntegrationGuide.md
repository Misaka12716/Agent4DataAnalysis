...
| [`ChunkedUploadFrontend.md`](ChunkedUploadFrontend.md) | **大文件分片上传**（init/part/complete、断点续传、各 target 迁移）；workbench 除外 |
...
| [`PsychAPI.md`](PsychAPI.md) | **2.1.4** 多维度分析与大模型 `/psych/*` 对接接口全文 |
| [`2.1.4FrontendIntegration.md`](2.1.4FrontendIntegration.md) | 2.1.4 联调入口（`/psych-app` Demo） |
...
| 数据接入 | 推荐分片上传 `POST /upload/chunked/*`（`target=session`，见 [`ChunkedUploadFrontend.md`](ChunkedUploadFrontend.md)）；过渡期仍可用 `POST /session/upload-excel`（已 deprecated）；删除 `DELETE /session/workspace-file`；可选项目 `raw/` 预置 + `POST /session/copy-from-project-raw` |
...
| 删除工作区文件 | DELETE | `/session/workspace-file` | body: `{session_id, relative_path}`，`data_delete` |
...
| `data_delete` | 数据删除 | 项目资产删除、会话工作区文件删除按钮 |
...
`bash scripts/init-platform.sh` 后在 `tests/fixtures/` 有横截面/纵向样本；另有 `table/large-dataset.csv`、`imaging/患者CT.dcm` 等可用于分片上传与多模态验收（见 [`ChunkedUploadFrontend.md`](ChunkedUploadFrontend.md) §10、[`Tests.md`](Tests.md)）。
